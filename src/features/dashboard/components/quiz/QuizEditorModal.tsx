...
export const QuizEditorModal: React.FC<QuizEditorModalProps> = ({
    ...
    // ⭐ جلب بيانات الكويز والأسئلة مباشرة من الباك إند
    ...
    const addQuestionMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            // ⭐ بمجرد النجاح، نطلب من المتصفح يجيب التحديث الجديد للأسئلة من الباك إند
            queryClient.invalidateQueries({ queryKey: ['examDetails', lessonId] });
        },
        onError: (error: any) => {
    ...
    const handleTypeChange = (type: number) => {
        const defaultOptions = type === 1 // True/False
    ...
    const handleUpdateOption = (index: number, field: 'text' | 'isCorrect', value: any) => {
        ...
        if (field === 'isCorrect' && value && currentQuestion.questionType !== 2) {
    ...
    const typeLabels: Record<number, string> = { 0: 'MCQ', 1: 'T/F', 2: 'Multi' };
            ...
            <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl w-full max-w-5xl h-[88vh] flex flex-col overflow-hidden shadow-2xl border border-slate-200 dark:border-[#2a2a2e]">
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-52 shrink-0 border-r border-slate-200 dark:border-[#2a2a2e] bg-slate-50 dark:bg-[#111111] flex flex-col p-3 gap-1">
                        ...
                        <button
                            onClick={() => {
                                if (!examIdRef.current) return toast.error('Save settings first to create the exam!');
                    ...
                    {activeTab === 'settings' && (
                        <div className="flex-1 overflow-y-auto p-6">
                            <div className="max-w-2xl flex flex-col gap-5">
                                ...
                                >
                                    {syncStatus === 'saved' && examIdRef.current
                                        ? <><CheckCircle size={16} /> Saved — Go to questions</>
                                        : isSaving
                                            ? 'Saving...'
                                        ...
                                        {currentQuestion.options?.map((opt, idx) => (
                                            ...
                                            >
                                                ...
                                                <input
                                                    ...
                                                    disabled={currentQuestion.questionType === 1}
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {questions.map((q, idx) => (
                                            <div key={q.id} className="bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-[#2a2a2e] rounded-xl p-3 group relative">
                                                {/* ⭐ التعامل مع الأسئلة اللي ممكن تكون null */}
