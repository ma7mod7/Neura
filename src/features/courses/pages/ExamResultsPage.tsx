...
import { AttemptUnderReviewState } from "../../analysis/components/AttemptUnderReviewState";
...
export default function ExamResultsPage() {
  ...
  const { data: results, isLoading: resultsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log("RAW attempt results:", JSON.stringify(data, null, 2));
      return data;
    },
    enabled: !!attemptId,
    staleTime: 1000 * 60 * 5,
  ...
  // Author redirect: fires once after data loads only if grades not published
  useEffect(() => {
    if (!resultsLoading && results && isAuthor && !results.areGradesPublished) {
      navigate(`/exam/${examId}/attempts/${attemptId}/review`, {
        replace: true,
      });
    }
  }, [resultsLoading, results, isAuthor, examId, attemptId, navigate]);
  ...
  if (!results) {
    return (
      <div className="h-screen bg-[#F8FAFC] dark:bg-[#0e0e10] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 dark:text-[#d0d0E0] font-medium">
          {t("examResults.couldNotLoad")}
        </p>
        <button
          onClick={() => navigate(-1)}
          className="text-[#0061EF] font-semibold hover:underline"
        >
          {t("examResults.goBack")}
        </button>
      </div>
    );
  }
  ...
  if (!results.areGradesPublished) {
    ...
    // Student: always show pending state
    return (
      <div className="h-screen bg-[#F8FAFC] dark:bg-[#0e0e10] flex flex-col">
        <div className="bg-white dark:bg-[#1A1A1A] border-b border-slate-200 dark:border-[#2a2a2e]">
          <div className="max-w-7xl mx-auto px-4 py-3.5">
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 text-slate-500 hover:text-[#0061EF] text-sm font-semibold"
            >
              <ArrowLeft size={18} className="rtl:rotate-180" />
              {t("examResults.backToCourse")}
            </button>
          </div>
        </div>
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="bg-white dark:bg-[#1A1A1A] rounded-3xl border border-slate-200 dark:border-[#2a2a2e] p-10 max-w-md w-full shadow-sm">
            <AttemptUnderReviewState
              attemptId={attemptId!}
  ...
  //  Grades published show results (both student and instructor) 
  const passed = results.isPassed ?? results.passed ?? false;
  const score = Math.round(results.scorePercentage ?? results.score ?? 0);
  const correctCount =
    results.questions?.filter((q: any) => q.isCorrect).length ?? 0;
  const totalCount = results.questions?.length ?? 0;
  const timeTaken =
    results.startedAt && results.submittedAt
      ? Math.round(
          (new Date(results.submittedAt).getTime() -
    ...
    <div className="h-screen bg-[#F8FAFC] dark:bg-[#0e0e10] font-inter flex flex-col overflow-hidden">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] border-b border-slate-200 dark:border-[#2a2a2e] shrink-0 z-20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3.5 flex items-center gap-4">
          ...
          <h1 className="text-base font-bold text-slate-800 dark:text-[#E0E0E0]">
            {t("examResults.title")}
          </h1>
        </div>
      </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-6 p-4 md:p-6 overflow-hidden">
        ...
        <div className="w-full lg:w-[400px] xl:w-[450px] shrink-0 flex flex-col gap-5 overflow-y-auto pe-1 pb-8 custom-scrollbar">
          {/* Hero result card */}
          <div
            className={`relative overflow-hidden rounded-3xl p-6 flex flex-col items-center text-center border shadow-sm shrink-0 ${
              passed
          ...
          >
            <h2 className="text-2xl font-black text-slate-800 dark:text-[#E0E0E0] mb-4 tracking-tight">
              {passed
                ? t("examResults.passedTitle")
                : t("examResults.failedTitle")}
            </h2>
            <div className="relative flex items-center justify-center mb-2">
              <svg className="w-32 h-32 transform -rotate-90">
                <circle
                  ...
                  cy="64"
                  r="56"
                  className="stroke-current text-slate-200 dark:text-[#2a2a2e]"
                  strokeWidth="10"
                  fill="transparent"
                />
                <circle
              ...
              <div className="absolute flex flex-col items-center">
                <span className="text-3xl font-black text-slate-800 dark:text-[#E0E0E0]">
                  {score}%
                </span>
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            {[
              ...
              <div
                key={i}
                className="bg-white dark:bg-[#1A1A1A] rounded-2xl p-4 border border-slate-200 dark:border-[#2a2a2e] shadow-sm flex items-center gap-3"
              >
                <div
                ...
                <div>
                  <p className="text-[11px] font-medium text-slate-500 dark:text-[#d0d0E0] uppercase">
                    {stat.label}
                  </p>
                  <p className="text-lg font-bold text-slate-800 dark:text-[#E0E0E0] leading-tight">
                    {stat.value}
                  </p>
                </div>
              </div>
          ...
          {!passed && results?.violationReason && (
            <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-500/20 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-red-500 uppercase tracking-wider mb-1">
                  {t("examResults.flagReason") ?? "Reason for Failure"}
                </p>
                <p className="text-sm text-red-700 dark:text-red-400 font-medium">
                  {results.violationReason}
                </p>
              </div>
            </div>
          )}
          ...
          {analytics && (
            <div className="bg-white dark:bg-[#1A1A1A] rounded-2xl border border-slate-200 dark:border-[#2a2a2e] p-5 shadow-sm shrink-0">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-sm text-slate-800 dark:text-[#E0E0E0]">
                  {t("examResults.performanceInsights")}
                </h3>
              </div>
              <div className="flex flex-col gap-4">
                <div>
                  <div className="flex justify-between text-xs font-bold mb-1.5">
                    <span className="text-slate-700 dark:text-[#E0E0E0]">
                      {t("examResults.yourScore")}
                    </span>
                    <span className="text-[#0061EF]">{score}%</span>
                  </div>
                  <div className="h-2 bg-slate-100 dark:bg-[#2a2a2e] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#0061EF] rounded-full"
                      style={{ width: `${score}%` }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs font-bold mb-1.5">
                    <span className="text-slate-500 dark:text-[#d0d0E0]">
                      {t("examResults.classAverage")}
                    </span>
                    <span className="text-slate-500 dark:text-[#d0d0E0]">
                  ...
                  <div className="h-2 bg-slate-100 dark:bg-[#2a2a2e] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-slate-400 dark:bg-[#4a4a4e] rounded-full"
                      style={{
                        width: `${Math.round(analytics.averageScore ?? 0)}%`,
                      }}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 mt-2">
                  <div className="text-center p-3 bg-slate-50 dark:bg-[#0e0e10] rounded-xl border border-slate-100 dark:border-[#2a2a2e]">
                    <p className="text-[10px] font-semibold text-slate-500 uppercase">
                      {t("examResults.highestScore")}
                    </p>
                    <p className="text-lg font-bold text-slate-800 dark:text-[#E0E0E0]">
                      {Math.round(analytics.highestScore ?? 0)}%
                    </p>
                  </div>
                  <div className="text-center p-3 bg-slate-50 dark:bg-[#0e0e10] rounded-xl border border-slate-100 dark:border-[#2a2a2e]">
                    <p className="text-[10px] font-semibold text-slate-500 uppercase">
                      {t("examResults.passRate")}
                    </p>
                    <p className="text-lg font-bold text-slate-800 dark:text-[#E0E0E0]">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-[#1A1A1A] rounded-3xl border border-slate-200 dark:border-[#2a2a2e] shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 dark:border-[#2a2a2e] bg-slate-50/50 dark:bg-[#111111] shrink-0 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-base text-slate-800 dark:text-[#E0E0E0]">
                  {t("examResults.breakdown")}
                </h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-4 custom-scrollbar">
              {results.questions.map((q: any, i: number) => {
                ...
                return (
                  <div
                    key={q.questionId ?? i}
                    className={`p-4 md:p-5 rounded-2xl border-2 transition-colors hover:shadow-md ${q.isCorrect ? "border-green-100 dark:border-green-500/20 bg-green-50/30 dark:bg-green-500/5" : "border-red-100 dark:border-red-500/20 bg-red-50/30 dark:bg-red-500/5"}`}
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1 ${q.isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                      >
                        {q.isCorrect ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                            {t("examResults.question", { number: i + 1 })}
                          </span>
                          <span
                            className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${q.isCorrect ? "bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400"}`}
                          >
                            {q.isCorrect
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                          <div className="p-3 rounded-xl bg-white dark:bg-[#1A1A1A] border border-slate-100 dark:border-[#2a2a2e] shadow-sm">
                            <span className="text-[11px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">
                              {t("examResults.yourAnswer")}
                            </span>
                            <span
                              className={`text-sm font-semibold ${q.isCorrect ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}
                            >
